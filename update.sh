...
do
   ...
   if [[ -z $dep || -z $home || -z $maintainer || -z $sponsor ]];then
       ...
       buildDEBIAN=0
   if [[ -z $dep ]]; then
       echo "Depiction: https://etiopia21.github.io/description.html?goto=${pkg}" >> ./debs/tmp/DEBIAN/control
       buildDEBIAN=1
   fi
   if [[ -z $home ]]; then
        echo "Homepage: https://etiopia21.github.io/" >> ./debs/tmp/DEBIAN/control
       buildDEBIAN=1
   fi
   if [[ -z $maintainer ]]; then
        echo "Maintainer: Lance2691 <argus2691@gmail.com>" >> ./debs/tmp/DEBIAN/control
       buildDEBIAN=1
   fi
   if [[ -z $sponsor ]]; then
        echo "Sponsor: Lance2691 <https://etiopia21.github.io>" >> ./debs/tmp/DEBIAN/control
       buildDEBIAN=1
   fi
   #binary 0 or 1
